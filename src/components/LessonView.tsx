...
interface LessonViewProps {
  ...
  setTheme?: React.Dispatch<
    React.SetStateAction<"light" | "dark" | "terminal">
  >;
  onNextLesson?: () => void;
  onPrevLesson?: () => void;
}
...
export default function LessonView({
  ...
  const [editorResult, setEditorResult] = useState<SQLRunnerResult | null>(
    null,
  );
  ...
  // Quiz States
  const [quizSection, setQuizSection] = useState<"mcq" | "tf" | "challenge">(
    "mcq",
  );
  ...
  const [isBookmarked, setIsBookmarked] = useState(
    userStats.bookmarks.includes(lesson.id),
  );
  ...
  useEffect(() => {
    ...
    // Clear starting visualizer state so user compiles their own solution
  ...
  const startVisualizer = () => {
    ...
    setIsActive(true);
    const startIndex =
      currentStepIndex >= vizSteps.length - 1 ? 0 : currentStepIndex;
  ...
  const runCustomQuery = () => {
    ...
    if (res.success) {
      ...
      const cleanInput = editorSql
        .trim()
        .replace(/\s+/g, " ")
        .replace(/;$/, "")
        .toUpperCase();
      const cleanExpected = lesson.expected_answer
        .trim()
        .replace(/\s+/g, " ")
        .replace(/;$/, "")
        .toUpperCase();
  ...
  const finishLesson = () => {
    ...
    if (isAlreadyCompleted) {
      setToast(
        "Review Mode: You've already mastered this mission and claimed its XP!",
      );
    } else {
      const earnedXp = 50;
  ...
  return (
    <div
      className="h-full bg-surface-bright flex flex-col overflow-hidden font-sans"
      id="lesson-workspace"
    >
      {/* Lesson Header Rail */}
      <div className="h-16 px-2 sm:px-4 md:px-8 border-b border-outline-variant bg-surface-container-lowest flex items-center justify-between shrink-0">
        <div className="flex items-center gap-1.5 sm:gap-3">
          ...
          <div className="min-w-0">
            <p className="text-[9px] uppercase font-bold font-mono text-primary tracking-wider truncate max-w-[80px] xs:max-w-[110px] sm:max-w-none">
              {isMobile
                ? `Lsn ${lesson.lesson_number}`
                : `Lesson ${lesson.lesson_number}`}{" "}
              • {lesson.query_type}
            </p>
            <h2
              className="text-xs sm:text-sm font-bold text-on-surface truncate max-w-[80px] xs:max-w-[120px] sm:max-w-xs md:max-w-md"
              title={lesson.title}
            >
              {lesson.title}
            </h2>
        ...
        <div className="flex items-center gap-1 sm:gap-2 shrink-0">
          ...
          <button
            ...
            title={
              isDrawerOpen ? "Hide Playground Drawer" : "Show Playground Drawer"
            }
          ...
          >
            <Terminal
              size={14}
              className={
                isDrawerOpen ? "text-primary" : "text-on-surface-variant"
              }
            />
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        >
          ...
          <div className="space-y-2">
            ...
            <h1 className="text-xl font-extrabold text-on-surface tracking-tight leading-snug">
              {lesson.title}
            </h1>
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              <h3 className="text-[9px] font-bold uppercase tracking-wider text-on-surface-variant font-mono">
                Target Query Block
              </h3>
              {!isDrawerOpen && (
                <button
            ...
            <div className="p-4 rounded-xl bg-[#11151C] text-slate-100 font-mono text-xs leading-relaxed border border-slate-800 shadow-inner relative select-all">
              <code className="block text-indigo-250 font-bold">
                {lesson.sql}
              </code>
            </div>
          </div>
            ...
            <div className="space-y-3 bg-surface-container-lowest p-4 rounded-xl border border-outline-variant/40">
              {lesson.line_by_line_explanation.map((line, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className="flex gap-3.5 items-start text-xs leading-relaxed text-on-surface-variant font-sans"
                  >
                    ...
                    <div className="space-y-1 pt-0.5 flex-1">
                      {explanationPart ? (
                        <>
                          ...
                          <p className="text-on-surface leading-relaxed text-[13px] font-medium pl-1">
                            {explanationPart}
                          </p>
                        </>
                      ) : (
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              {lesson.keyword_breakdown.map((kw, idx) => (
                <details
                  key={idx}
                  className="group p-4 rounded-xl border border-outline-variant/60 bg-surface-container-lowest hover:border-primary/40 transition-all [&_summary::-webkit-details-marker]:hidden"
                >
                  <summary className="flex items-center justify-between cursor-pointer font-extrabold text-[13px] text-on-surface font-mono select-none">
                    <div className="flex items-center gap-2">
                      <span className="text-primary font-mono bg-primary/5 px-2.5 py-1 rounded-md border border-primary/10">
                        {kw.keyword}
                      </span>
                    </div>
                    <div className="flex items-center gap-1.5 text-[11px] font-serif font-medium text-primary group-hover:underline">
                      ...
                      <ChevronRight
                        size={14}
                        className="transform group-open:rotate-90 transition-transform duration-200"
                      />
                    </div>
                  </summary>
                  ...
                  <div className="pt-4 space-y-4 border-t border-outline-variant/50 mt-3 text-xs leading-relaxed text-on-surface-variant font-sans">
                    ...
                    {kw.why_we_use_it && (
                      <div className="space-y-1 pl-1">
                        <span className="font-mono font-extrabold text-[9px] tracking-wider uppercase text-on-surface flex items-center gap-1.5">
                          <Lightbulb size={11} className="text-amber-500" /> Why
                          We Use It
                        </span>
                    ...
                    {kw.easy_example && (
                      <div className="bg-surface-container-low p-3 rounded-lg border border-outline-variant/30 space-y-1.5">
                        <span className="font-mono font-extrabold text-[9px] tracking-wider uppercase text-on-surface flex items-center gap-1.5">
                          <Code size={11} className="text-teal-500" /> In
                          Practice (Example)
                        </span>
        ...
        <AnimatePresence>
          {isDrawerOpen && (
            ...
            >
              ...
              <div className="h-14 px-4 border-b border-outline-variant bg-surface-container-lowest flex items-center justify-between shrink-0 font-mono text-xs">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="hidden sm:flex items-center gap-2 border border-outline-variant rounded px-2 py-1 bg-surface-container-low select-none">
                    ...
                    />
                    <span className="text-[9px] font-mono font-bold text-primary min-w-[24px] text-right">
                      {drawerWidth}%
                    </span>
                  </div>
              ...
              <div
                className="flex-1 overflow-y-auto md:overflow-hidden p-3.5 sm:p-5"
                id="active-tab-content-container"
              >
                {/* 1. VISUALIZER TAB */}
                {rightTab === "visualizer" && (
                  <div
                    className="flex flex-col gap-4 h-auto md:h-full md:overflow-hidden"
                    id="visualizer-panel"
                  >
                    ...
                    <div className="bg-[#11151C] border border-slate-800 rounded-xl overflow-hidden flex flex-col shadow-inner shrink-0">
                      ...
                      <div className="h-8 px-2 bg-slate-950 border-t border-slate-900 flex items-center gap-1.5 overflow-x-auto shrink-0 scrollbar-none">
                        <span className="text-[8px] font-mono font-bold text-slate-500 pr-1 select-none uppercase">
                          Suggest:
                        </span>
                        {[
                          "SELECT",
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:flex-1 md:overflow-hidden">
                      ...
                      <div className="bg-surface-container-lowest rounded-xl border border-outline-variant overflow-hidden flex flex-col justify-between min-h-[280px] md:min-h-0 md:flex-1">
                        <div className="h-10 px-3 bg-surface-container-low border-b border-outline-variant flex items-center justify-between">
                          ...
                          <span className="text-[9px] text-on-surface-variant font-mono font-bold">
                            {activeInputTable.rows.length} rows
                          </span>
                        </div>
                            ...
                            <thead>
                              <tr className="border-b border-outline-variant text-left text-on-surface-variant">
                                {activeInputTable.columns.map((col) => (
                                  <th
                                    key={col}
                                    className="p-2 font-bold tracking-tight text-on-surface-variant"
                                  >
                                    {col}
                                  </th>
                                ))}
                            ...
                            <tbody>
                              {activeInputTable.rows.map((row, rIdx) => {
                                return (
                                  <tr
                                    key={rIdx}
                                    className="border-b border-outline-variant/30 hover:bg-surface-container-low/50 transition-all duration-200"
                                  >
                                    {activeInputTable.columns.map((col) => (
                                      <td
                                        key={col}
                        ...
                        <div className="h-10 px-3 bg-slate-900 border-t border-slate-800 flex items-center text-[10px] font-mono text-slate-400 justify-between">
                          ...
                          <span className="text-[9px] font-mono font-extrabold text-emerald-500">
                            Oracle v19c DB Connected
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="bg-surface-container-lowest rounded-xl border border-outline-variant overflow-hidden flex flex-col min-h-[280px] md:min-h-0 md:flex-1">
                        <div className="h-10 px-3 bg-surface-container-low border-b border-outline-variant flex items-center justify-between">
                          ...
                          <span className="text-[8px] text-emerald-700 font-bold font-mono bg-emerald-500/10 px-1.5 py-0.5 rounded border border-emerald-500/20">
                            REALTIME
                          </span>
                        </div>
                          ...
                          editorResult.rows.length > 0 ? (
                            <table className="w-full text-[10px] font-mono border-collapse">
                              ...
                              <tbody>
                                {editorResult.rows.map((row, rIdx) => (
                                  <tr
                                    key={rIdx}
                                    className="border-b border-outline-variant/30 animate-in fade-in duration-200 hover:bg-surface-container-low/50"
                                  >
                                    {editorResult.columns.map((col) => (
                                      <td
                                        key={col}
                                        className="p-2 text-on-surface font-medium"
                                      >
                                        {String(row[col])}
                                      </td>
                                    ))}
                            ...
                            <div className="h-full flex flex-col items-center justify-center text-center p-4 space-y-2">
                              ...
                              <div className="space-y-0.5">
                                ...
                                <p className="text-[10px] text-on-surface-variant max-w-xs font-serif italic">
                                  Compile and execute a valid SQL query to
                ...
                {rightTab === "quiz" && (
                  <div
                    className="flex flex-col gap-4 h-auto md:h-full md:overflow-hidden text-left"
                    id="quiz-deck-panel"
                  >
                        ...
                        <div className="space-y-4">
                          {lesson.quiz.mcq.map((q, idx) => {
                            ...
                            return (
                              <div
                                key={idx}
                                className="p-4 rounded-xl bg-surface-container-lowest border border-outline-variant space-y-3"
                              >
                                ...
                                {submittedVal && (
                                  <div
                                    className={`p-3 rounded-lg border flex gap-2 text-[11px] leading-relaxed ${
                                      isCorrect
                                  ...
                                  >
                                    <div className="font-serif">
                                      <span className="font-mono font-bold uppercase text-[9px] block mb-0.5">
                      ...
                      {quizSection === "challenge" && (
                        <div className="p-4 rounded-xl bg-surface-container-lowest border border-outline-variant space-y-4">
                          <div className="space-y-1">
                            <span className="text-[9px] font-mono font-bold text-primary bg-primary-fixed px-2 py-0.5 rounded border border-outline-variant/30 uppercase tracking-wider">
                              Active Query Mission
                            </span>
                            <h4 className="text-sm font-bold text-on-surface font-sans pt-1">
                          ...
                          <div className="bg-surface-container-low p-3.5 rounded-lg border border-outline-variant text-[11px] font-mono leading-relaxed select-all">
                            <span className="font-bold text-on-surface uppercase text-[9px] tracking-wide block mb-1">
                          ...
                          <div className="bg-surface-container-low p-3.5 rounded-lg border border-outline-variant text-[11px] font-serif italic text-on-surface-variant space-y-1">
                            <span className="font-sans font-bold text-[9px] text-on-surface block uppercase not-italic tracking-wide">
                              How to solve:
                            </span>
                            <p>
                              1. Type your query solution in the Live SQL
                              Compiler on the{" "}
                              <span
                                className="font-bold text-primary cursor-pointer underline font-sans"
                                onClick={() => setRightTab("visualizer")}
                              >
                                Visualizer
                              </span>{" "}
                            ...
                            <p>
                              ...
                              to run it and verify the output.
                            </p>
                          </div>
                          ...
                          {codeChallengePassed ? (
                            <div className="p-3 rounded-lg border border-emerald-400 bg-emerald-500/5 flex gap-2 text-[11px] leading-relaxed text-emerald-800">
                              ...
                              />
                              <div>
                                <span className="font-mono font-bold uppercase text-[9px] block">
                                  Challenge Complete!
                                </span>
                                <p className="font-serif">
                                  Terrific job! The query output matches the
                                  target schemas perfectly.
                                </p>
                              </div>
                            </div>
                          ) : (
                            <div className="p-3 rounded-lg border border-outline-variant bg-surface-container-low flex gap-2 text-[11px] leading-relaxed text-on-surface-variant font-serif italic">
                              ...
                              />
                              <p>
                                Run query in Live SQL Compiler on the Visualizer
                                tab to complete the challenge.
                              </p>
                            </div>
                          )}
                        </div>
