...
export default function Playground() {
  ...
  const templates = [
    ...
    {
      label: "High Earners Sorted",
      sql: "SELECT first_name, salary FROM employees WHERE salary >= 17000 ORDER BY salary DESC;",
    },
    {
    ...
    {
      label: "Departments Catalog",
      sql: "SELECT department_name FROM departments;",
    },
    {
      label: "Jobs with High Limits",
      sql: "SELECT job_title, max_salary FROM jobs WHERE max_salary > 10000 ORDER BY max_salary DESC;",
    },
  ];
  ...
  return (
    <div
      className="space-y-8 animate-in fade-in duration-300 pb-24"
      id="playground-view"
    >
      {/* Visual Header */}
      <div className="text-center space-y-1">
        ...
        <p className="text-sm text-on-surface-variant font-serif italic">
          Draft arbitrary queries, sort, filter, and inspect structural
          compilation reports.
        </p>
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-[#11151C] border border-slate-800 rounded-xl overflow-hidden flex flex-col shadow-inner">
            ...
            <textarea
              ...
              className="w-full h-40 p-4 text-xs font-mono text-indigo-200 bg-[#11151C] border-none focus:outline-none focus:ring-0 leading-relaxed resize-none scrollbar-thin placeholder-slate-600"
            ...
            <div className="p-2 bg-slate-950 border-t border-slate-900 overflow-x-auto flex gap-1.5 shrink-0 scrollbar-none items-center">
              <span className="text-[8px] font-bold text-slate-500 flex items-center shrink-0 pr-1 select-none uppercase font-mono">
                templates:
              </span>
              {templates.map((temp) => (
                <button
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-surface-container-low p-4 rounded-xl border border-outline-variant">
            <p className="text-[11px] text-on-surface-variant font-serif italic max-w-sm leading-normal">
              Remember: Every query block requires a semicolon (;) to compile in
              standard Oracle PL/SQL environments.
            </p>
            <button
            ...
            <div className="flex-1 p-4 overflow-auto scrollbar-thin">
              {result ? (
                result.success ? (
                  result.rows.length > 0 ? (
                      ...
                      <thead>
                        <tr className="border-b border-outline-variant text-left text-on-surface-variant uppercase tracking-wider text-[9px]">
                          {result.columns.map((col) => (
                            <th
                              key={col}
                              className="p-2 font-bold text-on-surface"
                            >
                              {col}
                            </th>
                          ))}
                    ...
                    <div className="h-44 flex flex-col items-center justify-center text-center text-on-surface-variant space-y-2 font-serif">
                      ...
                      <p className="text-[11px] italic max-w-xs leading-normal">
                        Your query executed correctly, but no rows matched your
                        filter conditions.
                      </p>
                    </div>
                  )
                ...
                <div className="h-44 flex flex-col items-center justify-center text-center p-8 space-y-2 text-on-surface-variant/60 font-serif">
                  ...
                  <p className="text-[11px] italic max-w-xs leading-relaxed">
                    Enter your SQL commands in the terminal and click compile to
        ...
        <div className="space-y-6">
          ...
          <div className="bg-surface-container-lowest border border-outline-variant rounded-xl p-5 space-y-4">
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center text-[11px] font-mono">
                <span className="font-bold text-on-surface-variant uppercase text-[10px]">
                  Structure: {activeSchema.table_name}
                </span>
                <span className="font-bold text-primary">
                  {activeSchema.columns.length} cols
                </span>
              </div>
              ...
              <div className="space-y-1.5 max-h-48 overflow-y-auto pr-1">
                {activeSchema.columns.map((col) => {
                  ...
                  return (
                    <div
                      key={col}
                      className="p-2 bg-surface-container-low rounded border border-outline-variant/60 flex justify-between items-center text-[10px] font-mono"
                    >
          ...
          <div className="bg-surface-container-lowest p-5 rounded-xl border border-outline-variant space-y-2.5 font-serif">
            ...
            <div className="space-y-2 text-[11px] text-on-surface-variant italic leading-relaxed pl-1">
              <p>
                • WHERE filters are case-sensitive:{" "}
                <code className="font-mono font-bold bg-surface-container-low px-1 py-0.5 rounded not-italic">
                  first_name = 'Steven'
                </code>{" "}
                (capital S!).
              </p>
              <p>
                • Semicolons are required to segment multiple queries in
                real-world Oracle shells.
              </p>
              <p>
                • Avoid double quotes for string values, always use single
                quotes:{" "}
                <code className="font-mono font-bold bg-surface-container-low px-1 py-0.5 rounded not-italic">
                  'IT_PROG'
                </code>
